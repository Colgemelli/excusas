...
class SistemaExcusas {
    ...
    // STEPPER SIMPLE Y FUNCIONAL
    initSteppers() {
        this.currentStepExcusa = 1;
        this.currentStepPermiso = 1;
        this.maxSteps = 4;
        
        // Configurar eventos inmediatamente después de que el DOM esté listo
        document.addEventListener('DOMContentLoaded', () => {
            this.bindStepperEvents();
        });
        
        // También configurar después de que el objeto esté inicializado
        setTimeout(() => {
            this.bindStepperEvents();
        }, 500);
    }
    ...
    bindStepperEvents() {
        console.log('Configurando eventos del stepper...');
        
        // MÉTODO DIRECTO - Event listeners simples
        const nextBtnExcusa = document.getElementById('nextStepBtn');
        const prevBtnExcusa = document.getElementById('prevStepBtn');
        const nextBtnPermiso = document.getElementById('nextStepBtnPermiso');
        const prevBtnPermiso = document.getElementById('prevStepBtnPermiso');
        
        if (nextBtnExcusa) {
            console.log('Configurando botón siguiente excusa');
            // Remover listeners anteriores
            nextBtnExcusa.replaceWith(nextBtnExcusa.cloneNode(true));
            const newNextBtnExcusa = document.getElementById('nextStepBtn');
            
            newNextBtnExcusa.addEventListener('click', (e) => {
                e.preventDefault();
                e.stopPropagation();
                console.log('CLICK DETECTADO EN SIGUIENTE - EXCUSA');
                this.nextStep('excusa');
            });
            console.log('Event listener agregado al botón siguiente excusa');
        } else {
            console.error('Botón nextStepBtn no encontrado');
        }
        
        if (prevBtnExcusa) {
            prevBtnExcusa.replaceWith(prevBtnExcusa.cloneNode(true));
            const newPrevBtnExcusa = document.getElementById('prevStepBtn');
            newPrevBtnExcusa.addEventListener('click', (e) => {
                e.preventDefault();
                e.stopPropagation();
                console.log('CLICK DETECTADO EN ANTERIOR - EXCUSA');
                this.prevStep('excusa');
            });
        }
        
        if (nextBtnPermiso) {
            console.log('Configurando botón siguiente permiso');
            nextBtnPermiso.replaceWith(nextBtnPermiso.cloneNode(true));
            const newNextBtnPermiso = document.getElementById('nextStepBtnPermiso');
            
            newNextBtnPermiso.addEventListener('click', (e) => {
                e.preventDefault();
                e.stopPropagation();
                console.log('CLICK DETECTADO EN SIGUIENTE - PERMISO');
                this.nextStep('permiso');
            });
        }
        
        if (prevBtnPermiso) {
            prevBtnPermiso.replaceWith(prevBtnPermiso.cloneNode(true));
            const newPrevBtnPermiso = document.getElementById('prevStepBtnPermiso');
            newPrevBtnPermiso.addEventListener('click', (e) => {
                e.preventDefault();
                e.stopPropagation();
                console.log('CLICK DETECTADO EN ANTERIOR - PERMISO');
                this.prevStep('permiso');
            });
        }
        
        console.log('Eventos del stepper configurados');
    }
